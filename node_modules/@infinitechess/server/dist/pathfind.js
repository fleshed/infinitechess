import { PATHFIND_DETOUR_BUDGET, PATHFIND_MAX_NODES, bishopCanReach, coordEq, coordKey, distanceFor, enumerateMoves, } from '@infinitechess/shared';
/**
 ...
 */
export function pathfindNextMove(piece, goal, getOccupant, 
...
preferEmpty = null) {
    ...
    for (const m of moves) {
        if (coordEq(m, goal)) {
            if (DEBUG_PATHFIND)
                console.log(`[plan]   -> greedy reaches goal`);
            return m;
        }
    ...
    if (best && bestDist < currentDist) {
        if (DEBUG_PATHFIND)
            console.log(`[plan]   -> greedy step (${best.x},${best.y}) dist ${currentDist}->${bestDist}`);
        return best;
    }
    ...
    if (DEBUG_PATHFIND) {
        if (step)
            console.log(`[plan]   -> A* step (${step.x},${step.y})`);
        else
            console.log(`[plan]   -> A* FAILED, no path; goal will be cancelled`);
    }
    return step;
}
 ...
 */
export function isGoalReachable(piece, goal, getOccupant) {
    if (coordEq(piece.pos, goal))
        return true;
    if (piece.type === 'bishop' && !bishopCanReach(piece.pos, goal))
        return false;
    if (enumerateMoves(piece.type, piece.level, piece.pos, getOccupant).length === 0)
        return false;
    const startDist = distanceFor(piece.type, piece.pos, goal);
    return aStarFirstMove(piece, goal, getOccupant, startDist) !== null;
}
function aStarFirstMove(piece, goal, getOccupant, startDist, forbidden = null) {
    // Treat the start square as empty during search (we are leaving it).
    const occAt = (c) => (coordEq(c, piece.pos) ? 'empty' : getOccupant(c));
    const heuristic = (c) => distanceFor(piece.type, c, goal);
    ...
    pop() {
        ...
        if (this.data.length > 0) {
            ...
            while (true) {
                ...
                const r = 2 * i + 2;
                let smallest = i;
                if (l < n && this.key(this.data[l]) < this.key(this.data[smallest]))
                    smallest = l;
                if (r < n && this.key(this.data[r]) < this.key(this.data[smallest]))
                    smallest = r;
                if (smallest === i)
                    break;
