import {
  FORMATION_ELIGIBLE_TYPES,
  FORMATION_STAGGER_MAX,
  INTIMIDATION_FREEZE_TICKS,
  KING_THREAT_RADIUS,
  OSCILLATION_HISTORY,
  PIECE_VALUE,
  BUYABLE_TYPES,
  ...
  HOLE_REGEN_MS,
  RECAPTURE_LOCK_BUFFER_TICKS,
  REGROUP_RADIUS,
  ENGAGEMENT_SUPPORT_RADIUS,
  SPAWN_BASE_RADIUS,
  SPAWN_MAX_ATTEMPTS,
  SPAWN_RADIUS_PER_PLAYER,
  MIN_PAWN_SEPARATION,
  SLIDER_MAX_RANGE_L1,
  SLIDER_MAX_RANGE_L2,
  TELEGRAPH_BASE_MS,
  TELEGRAPH_PER_EXTRA_SQUARE_BEYOND_L1_MS,
  TELEGRAPH_PER_EXTRA_SQUARE_MS,
  TICK_MS,
...
function telegraphMsFor(type: PieceType, level: PieceLevel, from: Coord, to: Coord): number {
...
export class World {
  ...
  private pieceHistory = new Map<string, Coord[]>();
  /** Per-player viewport (world-square coords). Pieces outside are not streamed. */
  private viewports = new Map<PlayerId, C2S_Viewport>();
  ...
  private discoveredHidden = new Map<PlayerId, Set<string>>();
  ...
  private posIndex = new Map<string, PieceId>();
  ...
  private movingIds = new Set<PieceId>();
  ...
  private enemyCapCache = new Map<string, PieceId[]>();
  ...
  private static readonly CELL_SHIFT = 3; // cell size = 8 squares
  private cellIndex = new Map<string, Set<PieceId>>();
  ...
  private piecesByOwner = new Map<PlayerId, Set<PieceId>>();
  ...
  private addToCell(piece: Piece): void {
    const k = this.cellKey(piece.pos.x, piece.pos.y);
    let set = this.cellIndex.get(k);
    if (!set) { set = new Set(); this.cellIndex.set(k, set); }
    set.add(piece.id);
  }

  private removeFromCell(piece: Piece): void {
    const k = this.cellKey(piece.pos.x, piece.pos.y);
    const set = this.cellIndex.get(k);
    if (!set) return;
    set.delete(piece.id);
    if (set.size === 0) this.cellIndex.delete(k);
  }
  ...
  addPlayer(name: string): Player {
    ...
    for (const p of this.players.values()) inUse.add(p.color);
    let color = PLAYER_COLORS.find((c) => !inUse.has(c));
    if (!color) color = PLAYER_COLORS[this.players.size % PLAYER_COLORS.length]!;
    const player: Player = { id, name: name || `Player-${id.slice(0, 4)}`, color, currency: 0 };
  ...
  removePlayer(playerId: PlayerId): void {
    ...
    for (const [pid, piece] of [...this.pieces]) {
      if (piece.ownerId === playerId) this.removePieceById(pid);
    }
  }

  setViewport(playerId: PlayerId, vp: C2S_Viewport): void {
  ...
  requestMove(
    ...
    ignoreFriendlyIds?: Set<string>,
  ): { code: string; message: string } | null {
    const piece = this.pieces.get(pieceId);
    if (!piece) return { code: 'no_such_piece', message: 'Piece not found' };
    ...
    const occLookup = ignoreFriendlyIds && ignoreFriendlyIds.size > 0
      ? (c: Coord): Occupant => {
          ...
          if (occ && ignoreFriendlyIds.has(occ.id)) return 'empty';
          return this.getOccupant(c, piece.ownerId);
        }
      : (c: Coord): Occupant => this.getOccupant(c, piece.ownerId);
    if (!isGoalReachable(piece, goal, occLookup)) {
      if (DEBUG) console.log(`[req] REJECT ${piece.type} ${piece.id.slice(0,4)} -> (${goal.x},${goal.y}) unreachable`);
      return { code: 'unreachable', message: 'No path to that square right now' };
    }
    if (DEBUG) console.log(`[req] accept ${piece.type} ${piece.id.slice(0,4)} (${piece.pos.x},${piece.pos.y}) -> (${goal.x},${goal.y}) fid=${piece.formationId ? piece.formationId.slice(0,4) : 'none'}`);
    ...
    if (!piece.goal || !coordEq(piece.goal, goal)) this.pieceHistory.delete(pieceId);
    ...
    if (!deferQueue) {
      if (piece.formationId) {
        ...
        for (const peer of this.pieces.values()) {
          ...
          if (peer.formationId !== piece.formationId) continue;
          peers.push(peer.id);
          if (chebyshev(peer.pos, goal) <= 1) anyAdjacentToGoal = true;
        }
        if (!anyAdjacentToGoal && peers.length > 0) {
          ...
          if (DEBUG) console.log(`[leave] ${piece.type} ${piece.id.slice(0,4)} auto-leaves fid=${piece.formationId.slice(0,4)} (target non-adjacent)`);
        }
        ...
        for (const peer of this.pieces.values()) {
          if (peer.id === piece.id) continue;
          if (peer.ownerId !== piece.ownerId) continue;
          if (peer.squadId !== piece.squadId) continue;
          hasSquadmate = true;
          if (chebyshev(peer.pos, goal) <= 1) { anyAdjacentToGoal = true; break; }
        }
        if (hasSquadmate && !anyAdjacentToGoal) {
          piece.squadId = undefined;
          piece.slotOffset = undefined;
          piece.formationSlotId = undefined;
          if (DEBUG) console.log(`[detach] ${piece.type} ${piece.id.slice(0,4)} severs squad memory (target non-adjacent to squadmates)`);
        }
      }
    }
   ...
   */
  formationMove(playerId: PlayerId, pieceIds: string[], target: Coord): void {
    const pieces: Piece[] = [];
    for (const id of pieceIds) {
      const p = this.pieces.get(id);
      if (p && p.ownerId === playerId) pieces.push(p);
    }
    if (DEBUG) {
      const detail = pieces
    ...
    for (const p of pieces) {
      if (p.type === 'knight' || p.type === 'bishop') p.squadId ??= squadStamp;
    }
    ...
    const anchorSlotValid =
      anchor.slotOffset !== undefined &&
    ...
    for (const p of pieces) {
      ...
      const goal: Coord = { x: target.x + offX, y: target.y + offY };
      ...
      if (p.goal && coordEq(p.goal, goal)) p.formationGoal = { ...goal };
    }
  }

  /** Drop the goal on this piece. */
  cancelGoal(playerId: PlayerId, pieceId: string): void {
    const p = this.pieces.get(pieceId);
    if (!p || p.ownerId !== playerId) return;
    p.goal = undefined;
  }
   ...
   */
  leaveFormation(playerId: PlayerId, pieceId: string): { code: string; message: string } | null {
    const p = this.pieces.get(pieceId);
    if (!p) return { code: 'no_such_piece', message: 'Piece not found' };
    if (p.ownerId !== playerId) return { code: 'not_owner', message: 'You do not own this piece' };
    if (!p.formationId) return null;
    const peers: PieceId[] = [];
    for (const peer of this.pieces.values()) {
      if (peer.id === p.id) continue;
      if (peer.formationId !== p.formationId) continue;
      peers.push(peer.id);
    }
    ...
    p.slotOffset = undefined;
    p.formationSlotId = undefined;
    if (DEBUG) console.log(`[leave] ${p.type} ${p.id.slice(0,4)} explicit leave, excludes [${peers.map((id) => id.slice(0,4)).join(',')}]`);
    return null;
  }

  /** Sell a piece for its full PIECE_VALUE. Kings cannot be sold. */
  sellPiece(playerId: PlayerId, pieceId: string): { code: string; message: string } | null {
    const p = this.pieces.get(pieceId);
    if (!p) return { code: 'no_such_piece', message: 'Piece not found' };
    if (p.ownerId !== playerId) return { code: 'not_owner', message: 'You do not own this piece' };
    if (p.type === 'king') return { code: 'cannot_sell_king', message: 'Cannot sell your king' };
    const owner = this.players.get(playerId);
    if (owner) owner.currency += PIECE_VALUE[p.type];
  ...
  buyPiece(
    ...
    if (!player) return { code: 'no_such_player', message: 'Player not found' };
    if (!(BUYABLE_TYPES as readonly PieceType[]).includes(type)) {
    ...
    for (const p of this.pieces.values()) {
      ...
      if (chebyshev(p.pos, pos) <= 1) { adjacent = true; break; }
    }
    if (!adjacent) {
    ...
    // is disallowed.
    const occFn = (c: Coord): Occupant => this.getOccupant(c, playerId);
    const reach = enumerateMoves(type, 1, pos, occFn);
    for (const enemy of this.pieces.values()) {
      if (enemy.ownerId === playerId || enemy.type !== 'king') continue;
  ...
  private bombKeyAt(pos: Coord): string {
  ...
  placeBomb(
    ...
    for (const p of this.pieces.values()) {
      ...
      if (p.ownerId === playerId) {
        ...
        if (d < BOMB_PLACEMENT_ENEMY_RADIUS) { enemyTooClose = true; break; }
      }
    }
    if (enemyTooClose) {
      return {
  ...
  private tickHazards(): void {
    if (this.bombs.size > 0) {
      ...
      for (const [bombId, bomb] of [...this.bombs]) {
        if (bomb.fuseEndTick > this.tick) continue;
        ...
        for (let dx = -BOMB_BLAST_RADIUS; dx <= BOMB_BLAST_RADIUS; dx++) {
          for (let dy = -BOMB_BLAST_RADIUS; dy <= BOMB_BLAST_RADIUS; dy++) {
            ...
            if (victimId) this.removePieceById(victimId);
    ...
    if (this.holes.size > 0) {
      for (const [k, hole] of this.holes) {
        if (hole.regenEndTick <= this.tick) this.holes.delete(k);
      }
    }
  }
  ...
  step(): void {
    ...
    for (const p of this.pieces.values()) if (p.move) this.movingIds.add(p.id);
    ...
    for (const piece of this.pieces.values()) {
      ...
      if (piece.frozenUntilTick && piece.frozenUntilTick > this.tick) continue;
      if (piece.lockedUntilTick && piece.lockedUntilTick > this.tick) continue;
      ...
      // squad pushes on.
      if (!piece.move && !piece.goal) this.kingFollowScan(piece);
    }
    ...
    for (const piece of this.pieces.values()) {
      ...
      if (piece.goal) {
        if (piece.type === 'king') continue;
        if (!this.enemyCanCapture(piece.pos, piece.ownerId, piece.id)) continue;
    ...
    for (const [id, piece] of this.pieces) {
      if (piece.move && this.tick >= piece.move.endTick) pending.add(id);
    }
    let progress = true;
    while (progress) {
      progress = false;
    ...
    for (const id of pending) {
      const piece = this.pieces.get(id);
      if (piece && piece.move) this.commitMove(piece);
    }
  ...
  snapshotFor(playerId: PlayerId): WorldSnapshot {
    ...
    for (let cx = cMinX; cx <= cMaxX; cx++) {
      for (let cy = cMinY; cy <= cMaxY; cy++) {
        ...
        for (const id of cell) {
          const p = this.pieces.get(id);
          if (!p) continue;
          if (!pieceTouchesRect(p, minX, maxX, minY, maxY)) continue;
          seen.add(id);
          pieces.push(p);
        }
      }
    ...
    for (const b of this.bombs.values()) {
      if (b.pos.x >= minX && b.pos.x <= maxX && b.pos.y >= minY && b.pos.y <= maxY) bombs.push(b);
    }
    const holes: Hole[] = [];
    for (const h of this.holes.values()) {
      if (h.pos.x >= minX && h.pos.x <= maxX && h.pos.y >= minY && h.pos.y <= maxY) holes.push(h);
    }
    return {
      tick: this.tick,
      players: [...this.players.values()],
  ...
  private accrueResourceIncome(): void {
  ...
  private resourcesFor(
    ...
    const out: ResourceSquare[] = [];
    const seen = new Set<string>();
    const claimAt = (x: number, y: number): PlayerId | undefined => {
      for (const p of this.pieces.values()) {
        if (p.pos.x === x && p.pos.y === y) return p.ownerId;
      }
      return undefined;
    };
  ...
  private commitMove(piece: Piece): void {
    ...
    if (!target) {
      ...
      if (this.posIndex.get(fromKey) === piece.id) this.posIndex.delete(fromKey);
  ...
  private captureTo(attacker: Piece, victim: Piece, landing: Coord): void {
    const owner = this.players.get(attacker.ownerId);
    const bounty = PIECE_VALUE[victim.type];
    if (owner) owner.currency += bounty;
    ...
    if (this.posIndex.get(fromKey) === attacker.id) this.posIndex.delete(fromKey);
    ...
    if (defender) this.scheduleRecapture(defender, attacker);
  }
   ...
   */
  private chooseRecaptureDefender(victim: Piece, attacker: Piece): Piece | null {
    if (!victim.formationId) return null;
    ...
    const losOcc = (c: Coord): Occupant => {
      if (coordEq(c, attacker.pos)) return 'enemy';
      return this.getOccupant(c, victim.ownerId);
    };
    const ownerSet = this.piecesByOwner.get(victim.ownerId);
    if (ownerSet) for (const id of ownerSet) {
      ...
      if (!victim.squadId || p.squadId !== victim.squadId) continue;
      if (chebyshev(p.pos, victim.pos) > REGROUP_RADIUS) continue;
      if (canMoveTo(p, attacker.pos, losOcc)) candidates.push(p);
    }
    ...
    const acceptable = candidates.filter((d) => {
      if (d.type === 'king') {
        ...
        if (attacker.type === 'king') return false;
        return !this.enemyCanCapture(target, d.ownerId, d.id);
      }
      if (PIECE_VALUE[d.type] <= aVal) return true;
      return !this.enemyCanCapture(target, d.ownerId, d.id);
    });
    ...
    if (acceptable.length === 0) {
      if (DEBUG) console.log(`[trade] no favorable defender for ${victim.type} ${victim.id.slice(0,4)} vs ${attacker.type} ${attacker.id.slice(0,4)} @(${target.x},${target.y})`);
      return null;
    }
  ...
  private imminentPos(p: Piece): Coord {
  ...
  private enemyCanCapture(square: Coord, allyOwner: PlayerId, ignoreId?: string): boolean {
    const threats = this.enemyThreats(square, allyOwner);
    if (threats.length === 0) return false;
    if (!ignoreId) return true;
    for (const id of threats) if (id !== ignoreId) return true;
    return false;
  }
  ...
  private enemyThreats(square: Coord, allyOwner: PlayerId): PieceId[] {
    const key = `${square.x},${square.y}|${allyOwner}`;
    let threats = this.enemyCapCache.get(key);
    if (threats !== undefined) return threats;
    threats = [];
    ...
    for (let dx = -R; dx <= R; dx++) {
      for (let dy = -R; dy <= R; dy++) {
        ...
        for (const id of cell) {
          const p = this.pieces.get(id);
          if (!p || p.ownerId === allyOwner) continue;
          const pos = this.imminentPos(p);
          if (canReachForCapture(p.type, p.level, pos, square)) threats.push(p.id);
        }
      }
    }
   ...
   */
  private engagementScan(piece: Piece): boolean {
    ...
    type Candidate = { pos: Coord; victim: Piece; val: number };
    const candidates: Candidate[] = [];
    for (const m of moves) {
      const occ = this.getPieceAt(m);
      if (!occ || occ.ownerId === piece.ownerId) continue;
      if (!this.engagementFavorable(piece, occ)) continue;
    ...
    if (DEBUG) {
      console.log(
        `[engage] ${piece.type} ${piece.id.slice(0,4)}@(${piece.pos.x},${piece.pos.y}) -> capture ${pick.victim.type} ${pick.victim.id.slice(0,4)}@(${pick.pos.x},${pick.pos.y})`,
      );
    }
    return true;
  }
   ...
   */
  private regroupScan(piece: Piece): boolean {
    if (!piece.squadId) return false;
    ...
    if (resourceKindAt(piece.pos.x, piece.pos.y) !== null) return false;
    ...
    if (piece.slotOffset && piece.formationSlotId) {
      ...
      if (ownerSet) for (const id of ownerSet) {
        ...
        const p = this.pieces.get(id);
        if (!p) continue;
        if (p.squadId !== piece.squadId) continue;
        if (p.formationSlotId !== piece.formationSlotId) continue;
        if (!p.slotOffset) continue;
        if (excluded && excluded.includes(p.id)) continue;
        if (chebyshev(p.pos, piece.pos) > REGROUP_RADIUS * 2) continue;
        if (anchor === null || p.id < anchor.id) anchor = p;
      }
      if (anchor) {
        const slotTarget: Coord = {
          x: anchor.pos.x + (piece.slotOffset.x - anchor.slotOffset!.x),
          y: anchor.pos.y + (piece.slotOffset.y - anchor.slotOffset!.y),
        };
        if (coordEq(slotTarget, piece.pos)) return false; // already at slot
        ...
        if (!occ || occ.id === piece.id) {
          ...
          if (DEBUG) {
            console.log(
              `[regroup-slot] ${piece.type} ${piece.id.slice(0,4)}@(${piece.pos.x},${piece.pos.y}) -> slot (${slotTarget.x},${slotTarget.y}) anchor ${anchor.type} ${anchor.id.slice(0,4)}@(${anchor.pos.x},${anchor.pos.y})`,
            );
          }
          return true;
        }
    ...
    let unsafeTarget: Coord | null = null;
    let unsafeBest = Infinity;
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        if (dx === 0 && dy === 0) continue;
        const c: Coord = { x: nearest.pos.x + dx, y: nearest.pos.y + dy };
        const occ = this.getPieceAt(c);
        if (occ && occ.id !== piece.id) continue; // taken by someone else
        ...
        if (this.stepIsSafe(piece, c)) {
          ...
          unsafeBest = score; unsafeTarget = c;
        }
      }
    }
    if (!target) target = unsafeTarget;
    if (!target) return false; // squadmate completely walled in
    piece.goal = target;
    this.pieceHistory.delete(piece.id);
    if (DEBUG) {
      console.log(
        `[regroup] ${piece.type} ${piece.id.slice(0,4)}@(${piece.pos.x},${piece.pos.y}) -> adj squadmate ${nearest.type} ${nearest.id.slice(0,4)}@(${nearest.pos.x},${nearest.pos.y}) via (${target.x},${target.y}) dist=${nearestDist}`,
      );
    }
    return true;
  }
   ...
   */
  private hasSquadSupport(attacker: Piece, landing: Coord): boolean {
    if (!attacker.squadId) return false;
    const ownerSet = this.piecesByOwner.get(attacker.ownerId);
    if (!ownerSet) return false;
    for (const id of ownerSet) {
      if (id === attacker.id) continue;
      const p = this.pieces.get(id);
      if (!p || p.squadId !== attacker.squadId) continue;
      if (chebyshev(p.pos, landing) <= ENGAGEMENT_SUPPORT_RADIUS) return true;
    }
    return false;
  }
   ...
   */
  private engagementFavorable(attacker: Piece, target: Piece): boolean {
    if (attacker.type === 'king') {
      // Kings cannot capture other kings.
      if (target.type === 'king') return false;
      if (target.formationId) return false;
    ...
    if (!landingDefended) return true;
   ...
   */
  private stepIsSafe(piece: Piece, next: Coord): boolean {
    ...
    if (myVal > cheapestAttackerVal) return false; // losing trade
   ...
   */
  private kingSafetyScan(king: Piece): boolean {
    if (!king.formationId) return false;
    ...
    for (let dx = -R; dx <= R; dx++) {
      for (let dy = -R; dy <= R; dy++) {
        ...
        for (const id of cell) {
          const p = this.pieces.get(id);
          if (!p || p.ownerId === king.ownerId) continue;
          const pos = this.imminentPos(p);
          ...
          if (!canHit && d > KING_THREAT_RADIUS) continue;
          if (canHit) critical = true;
          if (d < threatDist) { threat = p; threatDist = d; }
        }
      }
    }
    if (!threat) return false;
    if ((king.move || king.goal) && !critical) return false;
    ...
    let safestPeer: Piece | null = null;
    let safestDist = kingToThreat;
    for (const p of this.pieces.values()) {
      if (p.id === king.id) continue;
      if (p.ownerId !== king.ownerId) continue;
      if (p.formationId !== king.formationId) continue;
      const d = chebyshev(p.pos, threatPos);
      if (d > safestDist) { safestPeer = p; safestDist = d; }
    }
    ...
    const dropStaleGoal = (reason: string) => {
      if (king.goal) {
        if (DEBUG) console.log(`[king-safety] CLEAR goal ${king.id.slice(0,4)} (${king.goal.x},${king.goal.y}) — ${reason}`);
        king.goal = undefined;
    ...
    const pickBest = (requireSafe: boolean): Coord | null => {
      let best: Coord | null = null;
      let bestScore = -Infinity;
      for (const c of neighbors) {
        const occ = this.getPieceAt(c);
        if (occ && occ.id !== king.id) continue; // square is held by someone else
        if (requireSafe && this.enemyCanCapture(c, king.ownerId, king.id)) continue;
    ...
    if (!target) {
      if (critical) dropStaleGoal('no reachable safe neighbor');
      return false;
    }
    ...
    let breaker: Piece | null = null;
    let breakerVal = Infinity;
    if (king.squadId) {
      const ownerSet = this.piecesByOwner.get(king.ownerId);
      if (ownerSet) for (const id of ownerSet) {
        if (id === king.id) continue;
        const p = this.pieces.get(id);
        if (!p) continue;
        if (p.type === 'king') continue;
        if (p.squadId !== king.squadId) continue;
        if (p.frozenUntilTick && p.frozenUntilTick > this.tick) continue;
        if (p.lockedUntilTick && p.lockedUntilTick > this.tick) continue;
        if (!canReachForCapture(p.type, p.level, p.pos, threatPos)) continue;
        const v = PIECE_VALUE[p.type];
        if (v < breakerVal) { breakerVal = v; breaker = p; }
      }
    }
    if (breaker) {
    ...
    if (king.squadId) {
      for (const p of this.pieces.values()) {
        if (p.id === king.id) continue;
        if (p.squadId !== king.squadId) continue;
        if (p === breaker) continue; // breaker keeps its check-capture goal
        if (p.move) continue; // already telegraphing; let it land
        p.goal = undefined;
    ...
    if (DEBUG) {
      ...
      console.log(
        `[king-safety] CRITICAL ${king.id.slice(0,4)}@(${king.pos.x},${king.pos.y}) threat=${threat.type} ${threat.id.slice(0,4)}${threatLoc} -> retreat (${target.x},${target.y}) adjacent ${safestPeer.type} ${safestPeer.id.slice(0,4)}@(${safestPeer.pos.x},${safestPeer.pos.y})`,
      );
    }
    return true;
  }
   ...
   */
  private kingFollowScan(king: Piece): boolean {
    if (king.type !== 'king') return false;
    if (king.move || king.goal) return false;
    if (king.frozenUntilTick && king.frozenUntilTick > this.tick) return false;
    if (king.lockedUntilTick && king.lockedUntilTick > this.tick) return false;
    if (!king.squadId) return false;
    const squadmates: Piece[] = [];
    const ownerSet = this.piecesByOwner.get(king.ownerId);
    if (ownerSet) for (const id of ownerSet) {
      if (id === king.id) continue;
      const p = this.pieces.get(id);
      if (!p) continue;
      if (p.squadId !== king.squadId) continue;
      if (chebyshev(p.pos, king.pos) <= REGROUP_RADIUS) squadmates.push(p);
    }
    if (squadmates.length === 0) return false;
    ...
    if (adjToSquadmate && safeNow) return false;
    ...
    for (const p of squadmates) {
      ...
      if (d < nsDist) { nsDist = d; nearestSquadmate = p; }
    }
    ...
    for (const p of this.pieces.values()) {
      ...
      if (d < nearestEnemyDist) { nearestEnemyDist = d; nearestEnemy = p; }
    }
    ...
    for (let dx = -1; dx <= 1; dx++) {
      for (let dy = -1; dy <= 1; dy++) {
        if (dx === 0 && dy === 0) continue;
        const c: Coord = { x: nearestSquadmate.pos.x + dx, y: nearestSquadmate.pos.y + dy };
        const occ = this.getPieceAt(c);
        if (occ && occ.id !== king.id) continue;
        if (this.enemyCanCapture(c, king.ownerId, king.id)) continue;
    ...
    if (coordEq(bestTarget, king.pos)) return false;
   ...
   */
  private scheduleRecapture(defender: Piece, capturer: Piece): void {
    defender.goal = { ...capturer.pos };
    const telegraphTicks = Math.max(1, Math.round(
      telegraphMsFor(defender.type, defender.level, defender.pos, capturer.pos) / TICK_MS,
    ));
  ...
  private removePieceById(id: string): void {
    const piece = this.pieces.get(id);
    if (piece) {
      const k = this.posKey(piece.pos.x, piece.pos.y);
      if (this.posIndex.get(k) === id) this.posIndex.delete(k);
      this.removeFromCell(piece);
      const ownerSet = this.piecesByOwner.get(piece.ownerId);
      if (ownerSet) {
        ownerSet.delete(id);
        if (ownerSet.size === 0) this.piecesByOwner.delete(piece.ownerId);
      }
   ...
   */
  private resolveMutualAttacks(): void {
    const arriving: Piece[] = [];
    for (const piece of this.pieces.values()) {
      if (piece.move && piece.move.endTick <= this.tick) arriving.push(piece);
    }
    const handled = new Set<string>();
    for (let i = 0; i < arriving.length; i++) {
      const a = arriving[i]!;
      if (handled.has(a.id) || !this.pieces.has(a.id)) continue;
      for (let j = i + 1; j < arriving.length; j++) {
        const b = arriving[j]!;
        if (handled.has(b.id) || !this.pieces.has(b.id)) continue;
        ...
        if (coordEq(a.move!.to, b.pos) && coordEq(b.move!.to, a.pos)) {
          ...
          if (av > bv) {
            ...
            a.move = undefined;
            if (a.goal && coordEq(a.pos, a.goal)) a.goal = undefined;
            this.recordHistoryAndCheckLoop(a, aFrom);
          } else if (bv > av) {
            const bFrom = { ...b.move!.from };
            this.captureTo(b, a, a.pos);
            b.move = undefined;
            if (b.goal && coordEq(b.pos, b.goal)) b.goal = undefined;
  ...
  private recordHistoryAndCheckLoop(piece: Piece, from?: Coord): void {
    ...
    if (!hist) {
      ...
      if (from && !coordEq(from, piece.pos)) hist.push({ ...from });
    }
    hist.push({ ...piece.pos });
    if (hist.length > OSCILLATION_HISTORY) hist.splice(0, hist.length - OSCILLATION_HISTORY);
    ...
    for (let i = 0; i < hist.length - 1; i++) {
      if (coordEq(hist[i]!, last)) {
        if (DEBUG) console.log(`[loop] CANCEL ${piece.type} ${piece.id.slice(0,4)} revisits (${last.x},${last.y}); goal dropped`);
        piece.goal = undefined;
        hist.length = 0;
        return;
      }
  ...
  private tryQueueNextMove(piece: Piece): void {
    ...
    if (piece.formationId && this.formationLeaderShouldWait(piece)) return;
    ...
    const preferEmpty = (c: Coord): boolean => !this.isFriendlyCurrentlyAt(c, piece.id, piece.ownerId);
    ...
    const onFormationMarch =
      !!piece.formationGoal && coordEq(piece.formationGoal, goal);
    ...
    if (this.isFriendlyTargeting(next, piece.ownerId, piece.id)) return;
    ...
    if (piece.type === 'king' && this.enemyCanCapture(next, piece.ownerId, piece.id)) {
      ...
      for (let dx = -1; dx <= 1; dx++) {
        for (let dy = -1; dy <= 1; dy++) {
          if (dx === 0 && dy === 0) continue;
          const c: Coord = { x: piece.pos.x + dx, y: piece.pos.y + dy };
          const occ = this.getPieceAt(c);
          if (occ && occ.ownerId === piece.ownerId) continue;
          if (this.enemyCanCapture(c, piece.ownerId, piece.id)) continue;
          const d = chebyshev(c, goal);
          if (d < altDist) { altDist = d; alt = c; }
        }
      }
      if (!alt) {
        if (DEBUG) console.log(`[king-check] ${piece.id.slice(0,4)} refuses step (${next.x},${next.y}) toward goal (${goal.x},${goal.y}) — no safe alternative, holding`);
        return;
      }
      if (DEBUG) console.log(`[king-check] ${piece.id.slice(0,4)} reroute (${next.x},${next.y}) -> (${alt.x},${alt.y}) — original step in check`);
      next = alt;
      if (this.isFriendlyTargeting(next, piece.ownerId, piece.id)) return;
    }
    ...
    if (piece.type !== 'king' && !coordEq(next, goal) && !this.stepIsSafe(piece, next)) {
      const occFor = (c: Coord) => this.getOccupant(c, piece.ownerId);
      const alts = enumerateMoves(piece.type, piece.level, piece.pos, occFor);
      const currentDist = distanceFor(piece.type, piece.pos, goal);
      let safer: Coord | null = null;
      let saferDist = currentDist;
      for (const c of alts) {
        if (coordEq(c, next)) continue;
        if (this.isFriendlyTargeting(c, piece.ownerId, piece.id)) continue;
        if (!this.stepIsSafe(piece, c)) continue;
        const d = distanceFor(piece.type, c, goal);
        if (d < saferDist) { saferDist = d; safer = c; }
      }
      if (!safer) {
        if (DEBUG_SAFETY) console.log(`[safety] ${piece.type} ${piece.id.slice(0,4)} refuses unsafe step (${next.x},${next.y}) toward (${goal.x},${goal.y}) — holding`);
        return;
      }
      if (DEBUG_SAFETY) console.log(`[safety] ${piece.type} ${piece.id.slice(0,4)} reroute (${next.x},${next.y}) -> (${safer.x},${safer.y}) — original unsafe`);
      next = safer;
      if (this.isFriendlyTargeting(next, piece.ownerId, piece.id)) return;
    }
  ...
  private friendlyVacatingEndTick(c: Coord, ownerId: PlayerId): number {
    let best = 0;
    for (const id of this.movingIds) {
      const piece = this.pieces.get(id);
      if (!piece || !piece.move || piece.ownerId !== ownerId) continue;
      if (!coordEq(piece.move.from, c)) continue;
      if (piece.move.endTick > best) best = piece.move.endTick;
    }
    return best;
  }
   ...
   */
  private recomputeFormations(): void {
    ...
    for (const p of this.pieces.values()) {
      if (!p.formationLeft || p.formationLeft.length === 0) continue;
      p.formationLeft = p.formationLeft.filter((id) => {
        const peer = this.pieces.get(id);
        return peer !== undefined && chebyshev(peer.pos, p.pos) <= 1;
      });
      if (p.formationLeft.length === 0) p.formationLeft = undefined;
    }

    const parent = new Map<PieceId, PieceId>();
    for (const p of this.pieces.values()) {
      p.formationId = undefined;
      parent.set(p.id, p.id);
    }
    const find = (a: PieceId): PieceId => {
      let r = a;
    ...
    const union = (a: PieceId, b: PieceId): void => {
      const ra = find(a);
      const rb = find(b);
      if (ra !== rb) parent.set(ra, rb);
    };

    const eligible = new Set<PieceType>(FORMATION_ELIGIBLE_TYPES);
    ...
    for (const [ownerId, ids] of this.piecesByOwner) {
      ...
      for (const id of ids) {
        const p = this.pieces.get(id);
        if (!p || !eligible.has(p.type)) continue;
        owned.push(p);
        pieces.push(p);
      }
      void ownerId;
      for (let i = 0; i < owned.length; i++) {
        ...
        for (let j = i + 1; j < owned.length; j++) {
          ...
          if (a.formationLeft?.includes(b.id)) continue;
          if (b.formationLeft?.includes(a.id)) continue;
          if (this.formationLinked(a, b)) union(a.id, b.id);
        }
      }
    }

    const groups = new Map<PieceId, PieceId[]>();
    for (const p of pieces) {
      const r = find(p.id);
    ...
    for (const [, members] of groups) {
      ...
      for (const id of members) {
        const p = this.pieces.get(id);
        if (!p) continue;
        if (p.move || p.goal) { atRest = false; break; }
        if (p.formationSlotId !== groupId || p.slotOffset === undefined) {
          stale = true;
        }
      }
      if (!atRest || !stale) continue;
    ...
    if (DEBUG) {
      ...
      for (const p of this.pieces.values()) {
        ...
        if (prev !== p.formationId) { changed = true; break; }
      }
      if (!changed) {
        // Also detect pieces that disappeared
        for (const id of this.lastFormationIds.keys()) {
          if (!this.pieces.has(id)) { changed = true; break; }
        }
      }
      if (changed) {
  ...
  private formationLinked(a: Piece, b: Piece): boolean {
    ...
    if (((aPawn && bHeavy) || (bPawn && aHeavy)) && chebyshev(a.pos, b.pos) === 1) return true;
    // Recapture eligibility either way.
    if (canReachForCapture(a.type, a.level, a.pos, b.pos)) return true;
    if (canReachForCapture(b.type, b.level, b.pos, a.pos)) return true;
    return false;
  }
   ...
   */
  private formationPreferredStep(piece: Piece): { step: Coord | null; pending: boolean } {
    if (!piece.goal) return { step: null, pending: false };
    const dxRaw = piece.goal.x - piece.pos.x;
    const dyRaw = piece.goal.y - piece.pos.y;
    if (dxRaw === 0 && dyRaw === 0) return { step: null, pending: false };
    ...
    if (piece.formationId || piece.type === 'pawn' || piece.type === 'rook') {
      if (Math.abs(dxRaw) >= Math.abs(dyRaw)) sy = 0;
      else sx = 0;
    }
    if (sx === 0 && sy === 0) return { step: null, pending: false };
    const next: Coord = { x: piece.pos.x + sx, y: piece.pos.y + sy };
    ...
    for (const other of this.pieces.values()) {
      ...
      if (!other.move && coordEq(other.pos, next)) {
        ...
        // Enemy occupant.
        if (pawnOrthoBlocked) return { step: null, pending: false };
        return { step: next, pending: false };
      }
      if (other.move && coordEq(other.move.to, next)) {
        if (other.ownerId === piece.ownerId) return { step: null, pending: false };
        if (pawnOrthoBlocked) return { step: null, pending: false };
        return { step: next, pending: false };
      }
   ...
   */
  private formationLeaderShouldWait(piece: Piece): boolean {
    const fid = piece.formationId;
    if (!fid || !piece.goal) return false;
    let othersExist = false;
    let maxOtherRemaining = 0;
    for (const other of this.pieces.values()) {
      if (other.id === piece.id || other.formationId !== fid) continue;
      if (!other.goal) continue;
      othersExist = true;
      const r = chebyshev(other.pos, other.goal);
      if (r > maxOtherRemaining) maxOtherRemaining = r;
    }
   ...
   */
  private getOccupant(c: Coord, viewerOwnerId: PlayerId): Occupant {
    ...
    if (this.holes.has(k)) return 'blocked';
    const idAtPos = this.posIndex.get(k);
    if (idAtPos) {
      const p = this.pieces.get(idAtPos)!;
      if (!p.move) return p.ownerId === viewerOwnerId ? 'friendly' : 'enemy';
      ...
      if (p.ownerId !== viewerOwnerId) return 'enemy';
    }
    ...
    for (const id of this.movingIds) {
      if (id === idAtPos) continue;
      const p = this.pieces.get(id);
      if (!p || !p.move || p.ownerId !== viewerOwnerId) continue;
      if (coordEq(p.move.to, c)) return 'friendly';
    }
    return 'empty';
  }
  ...
  private isFriendlyCurrentlyAt(c: Coord, excludeId: string, ownerId: PlayerId): boolean {
  ...
  private isFriendlyTargeting(c: Coord, ownerId: PlayerId, excludePieceId: string): boolean {
    ...
    if (idAtPos && idAtPos !== excludePieceId) {
      const p = this.pieces.get(idAtPos);
      if (p && p.ownerId === ownerId && !p.move) return true;
    }
    // Friendly telegraphing INTO `c`: scan only movingIds, small set.
    for (const id of this.movingIds) {
      if (id === excludePieceId) continue;
      const p = this.pieces.get(id);
      if (!p || !p.move || p.ownerId !== ownerId) continue;
      if (coordEq(p.move.to, c)) return true;
    }
    return false;
  }

  private getPieceAt(c: Coord): Piece | undefined {
    const id = this.posIndex.get(this.posKey(c.x, c.y));
    return id ? this.pieces.get(id) : undefined;
  }

  private spawnPiece(ownerId: PlayerId, type: PieceType, pos: Coord): Piece {
    const piece: Piece = {
      id: randomUUID(),
      ownerId,
      type,
    ...
    if (!ownerSet) { ownerSet = new Set(); this.piecesByOwner.set(ownerId, ownerSet); }
    ownerSet.add(piece.id);
    return piece;
  }

  private findSpawnCenter(): Coord {
    ...
    for (let attempt = 0; attempt < SPAWN_MAX_ATTEMPTS; attempt++) {
      ...
      if (this.canFitSpawn(c)) return c;
    }
    for (let r = 1; r < 200; r++) {
      for (let x = -r; x <= r; x++) {
        for (let y = -r; y <= r; y++) {
          if (Math.max(Math.abs(x), Math.abs(y)) !== r) continue;
          if (this.canFitSpawn({ x, y })) return { x, y };
        }
      }
    }
    return { x: 0, y: 0 };
  ...
  private canFitSpawn(center: Coord): boolean {
    const cells: Coord[] = [
      center,
    ...
    for (const c of cells) {
      if (this.getPieceAt(c)) return false;
      // Holes are impassable terrain — a fresh spawn must never overlap one.
      if (this.holes.has(this.posKey(c.x, c.y))) return false;
    }
    ...
    for (const p of this.pieces.values()) {
      ...
      for (const c of pawnCells) {
        if (chebyshev(p.pos, c) < MIN_PAWN_SEPARATION) return false;
      }
    }
    return true;
  }
...
function pieceTouchesRect(
  ...
  const inRect = (c: Coord) => c.x >= minX && c.x <= maxX && c.y >= minY && c.y <= maxY;
  if (inRect(piece.pos)) return true;
  if (piece.move) {
    if (inRect(piece.move.from) || inRect(piece.move.to)) return true;
  }
  return false;
}
